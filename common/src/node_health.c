...
static void transition_state(node_health_record_t *r, node_health_state_t new_state)
{
    ...
    /* When a node disappears, usually clear stale STATUS_REPORT flags.
     * Exception: keep BLE_OTA latched briefly because OTA mode intentionally
     * disables ESP-NOW, so OFFLINE right after ENTER_BT_OTA is expected. */
    if (new_state == NODE_STATE_OFFLINE || new_state == NODE_STATE_UNKNOWN) {
        uint32_t now = now_ms();
        bool keep_ota = ((r->status_flags & OPENDASH_STATUS_FLAG_BLE_OTA) != 0) &&
                        (now < r->ota_latch_until_ms);
        r->status_flags = keep_ota ? OPENDASH_STATUS_FLAG_BLE_OTA : 0;
    }
...
{
    if (!s_initialized || node >= OPENDASH_NODE_COUNT) return;

    xSemaphoreTake(s_mutex, portMAX_DELAY);
    node_health_record_t *r = &s_records[node];
    uint32_t now = now_ms();

    if (flags & OPENDASH_STATUS_FLAG_BLE_OTA) {
        r->ota_latch_until_ms = now + NODE_HEALTH_BLE_OTA_LATCH_MS;
    } else if (now >= r->ota_latch_until_ms) {
        r->ota_latch_until_ms = 0;
    }

    if (r->status_flags != flags) {
        ESP_LOGI(TAG, "%s: status_flags 0x%02X -> 0x%02X",
                 NODE_NAMES[(int)node], r->status_flags, flags);
...
{
    if (!s_initialized || node >= OPENDASH_NODE_COUNT) return 0;
    ...
    /* Expire stale OTA latch and clear synthetic flag if needed. */
    if ((r->status_flags & OPENDASH_STATUS_FLAG_BLE_OTA) &&
        now >= r->ota_latch_until_ms &&
        r->state != NODE_STATE_ONLINE && r->state != NODE_STATE_DEGRADED) {
        r->status_flags &= (uint8_t)~OPENDASH_STATUS_FLAG_BLE_OTA;
    }
...
bool node_health_any_in_ble_ota(opendash_node_t *out_node)
{
    ...
    for (int i = 0; i < OPENDASH_NODE_COUNT; i++) {
        if (s_records[i].status_flags & OPENDASH_STATUS_FLAG_BLE_OTA) {
            if (out_node) *out_node = (opendash_node_t)i;
            return true;
        }
    }
    return false;
