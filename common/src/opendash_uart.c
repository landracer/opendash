...
/* ── OBD-II summary block (35 bytes starting at offset 58) ──────────── */
#define MD_OFF_OBD2       58     /* Start of OBD2 summary block */
#define MD_OFF_OBD2_FLAGS 58     /* uint8:  flags (bit0=ELM ready, bit1=DTCs) */
#define MD_OFF_OBD2_RPM   59     /* int16:  RPM (unscaled, integer RPM) */
#define MD_OFF_OBD2_SPD   61     /* int16:  Speed × 100 */
#define MD_OFF_OBD2_CLT   63     /* int16:  Coolant temp × 100 */
#define MD_OFF_OBD2_LOAD  65     /* int16:  Engine load × 100 */
#define MD_OFF_OBD2_MAP   67     /* int16:  Intake MAP × 100 */
#define MD_OFF_OBD2_TPS   69     /* int16:  Throttle × 100 */
#define MD_OFF_OBD2_IAT   71     /* int16:  Intake temp × 100 */
#define MD_OFF_OBD2_MAF   73     /* int16:  MAF rate × 10 */
#define MD_OFF_OBD2_ADV   75     /* int16:  Timing advance × 100 */
#define MD_OFF_OBD2_STFT  77     /* int16:  STFT bank1 × 100 */
#define MD_OFF_OBD2_LTFT  79     /* int16:  LTFT bank1 × 100 */
#define MD_OFF_OBD2_FP    81     /* int16:  Fuel pressure × 10 */
#define MD_OFF_OBD2_BARO  83     /* int16:  Baro pressure × 100 */
#define MD_OFF_OBD2_OIL   85     /* int16:  Oil temp × 100 */
#define MD_OFF_OBD2_VOLT  87     /* int16:  Ctrl voltage × 100 */
#define MD_OFF_OBD2_FUEL  89     /* int16:  Fuel level × 100 */
...
static void uart_rx_task(void *arg)
{
    ...
    while (1) {
        ...
        /* Step 2: Read TAG byte — accept 0x5F (MD frame) or skip 0x4F (OBD2) */
        n = uart_read_bytes(MD_UART_PORT, &rx_byte, 1, pdMS_TO_TICKS(50));
        if (n != 1) continue;

        if (rx_byte == 0x4F) {
            /* OBD2 standalone frame — variable length, terminated by ETX.
             * The same OBD2 data is already embedded in the MD frame (bytes 58-92),
             * so we just drain this frame until ETX and continue. */
            uint8_t drain;
            int64_t drain_start = esp_timer_get_time();
            while ((esp_timer_get_time() - drain_start) / 1000 < 200) {
                int dn = uart_read_bytes(MD_UART_PORT, &drain, 1, pdMS_TO_TICKS(50));
                if (dn == 1 && drain == MD_ETX) break;
            }
            continue;
        }

        if (rx_byte == 0x44) {
            /* DTC frame from MD: TAG(0x44) count(1) {code[5]}×N ETX(0x03)
             * Variable length: 1 + count*5 bytes before ETX */
            uint8_t dtc_buf[96];
            int64_t dtc_start = esp_timer_get_time();
            int dtc_idx = 0;
            while ((esp_timer_get_time() - dtc_start) / 1000 < 300 &&
                   dtc_idx < (int)sizeof(dtc_buf) - 1) {
                uint8_t db;
                int dn = uart_read_bytes(MD_UART_PORT, &db, 1, pdMS_TO_TICKS(50));
                if (dn == 1) {
                    if (db == MD_ETX) break;
                    dtc_buf[dtc_idx++] = db;
                }
            ...
            if (dtc_idx >= 1) {
                uint8_t count = dtc_buf[0];
                if (count > OPENDASH_MAX_DTCS) count = OPENDASH_MAX_DTCS;
                int expected = 1 + count * 5;
                if (dtc_idx >= expected) {
                    portENTER_CRITICAL(&s_data_lock);
                    s_latest_data.dtc_count = count;
                    s_latest_data.dtc_data_valid = true;
                    for (int i = 0; i < count; i++) {
                        memcpy(s_latest_data.dtc_codes[i], &dtc_buf[1 + i * 5], 5);
                        s_latest_data.dtc_codes[i][5] = '\0';
                    }
                    portEXIT_CRITICAL(&s_data_lock);
                    ESP_LOGI(TAG, "DTC frame: %d codes", count);
                }
            }
            continue;
        }
        ...
        if (rx_byte == 0x56) {
            /* VIN frame from MD: TAG(0x56) vin[17] valid(1) ETX(0x03) */
            uint8_t vin_buf[20];
            int vin_n = uart_read_bytes(MD_UART_PORT, vin_buf, 19,
                                         pdMS_TO_TICKS(200));
            if (vin_n >= 19 && vin_buf[18] == MD_ETX) {
                portENTER_CRITICAL(&s_data_lock);
                memcpy(s_latest_data.vin, vin_buf, 17);
                s_latest_data.vin[17] = '\0';
                s_latest_data.vin_valid = (vin_buf[17] == 1);
                portEXIT_CRITICAL(&s_data_lock);
                ESP_LOGI(TAG, "VIN frame: %.17s valid=%d",
                         s_latest_data.vin, s_latest_data.vin_valid);
            }
            continue;
        }

        if (rx_byte == 0x4D) {
            /* MIL status frame: TAG(0x4D) mil_on(1) dtc_count(1) ETX(0x03) */
            uint8_t mil_buf[3];
            int mil_n = uart_read_bytes(MD_UART_PORT, mil_buf, 3,
                                         pdMS_TO_TICKS(100));
            if (mil_n >= 3 && mil_buf[2] == MD_ETX) {
                portENTER_CRITICAL(&s_data_lock);
                s_latest_data.mil_on = (mil_buf[0] != 0);
                s_latest_data.mil_dtc_count = mil_buf[1];
                portEXIT_CRITICAL(&s_data_lock);
                ESP_LOGI(TAG, "MIL frame: on=%d count=%d",
                         mil_buf[0], mil_buf[1]);
            }
            continue;
        }

        if (rx_byte != MD_BINARY_TAG) {
            ESP_LOGW(TAG, "STX found but TAG=0x%02X (expected 0x5F/0x44/0x56/0x4D)", rx_byte);
...
void opendash_uart_suspend(void) {
    if (s_uart_task) {
        vTaskSuspend(s_uart_task);
        uart_driver_delete(MD_UART_PORT);
        ESP_LOGW(TAG, "UART RX task suspended (BLE OTA)");
    }
}
...
bool opendash_uart_tx_available(void)
{
#if !OPENDASH_MULTIDISPLAY_CONNECTION
    return false;
#else
    return (OPENDASH_UART_TX_PIN >= 0 && s_status != OPENDASH_UART_DISABLED);
#endif
}

bool opendash_uart_send_command(uint8_t cmd)
{
#if !OPENDASH_MULTIDISPLAY_CONNECTION
    return false;
#else
    if (OPENDASH_UART_TX_PIN < 0) {
        ESP_LOGW(TAG, "Cannot send command: TX pin not configured");
        return false;
    }

    /* Frame: STX CMD ETX */
    uint8_t buf[3] = { 0x02, cmd, 0x03 };
    int written = uart_write_bytes(MD_UART_PORT, buf, 3);
    if (written == 3) {
        ESP_LOGI(TAG, "Sent command 0x%02X to multidisplay", cmd);
        return true;
    }
