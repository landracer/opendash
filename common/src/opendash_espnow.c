...
/**
 * @brief Send callback — logs delivery failures and notifies health monitor.
 *
                             ...
                             esp_now_send_status_t status)
{
    ...
    /* Notify the send status hook (node health ACK/NACK tracking) */
    if (s_send_status_cb) {
        s_send_status_cb(info->des_addr, (status == ESP_NOW_SEND_SUCCESS));
    }
...
esp_err_t opendash_espnow_init(opendash_node_t self_node)
{
    ...
    /* TX power for the dashboard RF bus. Units are 0.25 dBm steps
     * (range 8..84 -> 2..21 dBm). 2 dBm (8) proved too weak: slave->master
     * frames (e.g. parachute 0x95 echo, L2 ACKs) failed to reach the center,
     * producing a one-way link and TX-queue NO_MEM quarantine storms.
     * 13 dBm (52) restores a healthy bidirectional margin. */
