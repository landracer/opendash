...
#define OPENDASH_DEMO_DATA  0
#endif

/* ── Maximum DTC codes from ELM327 ───────────────────────────────────── */
#define OPENDASH_MAX_DTCS   16
#define OPENDASH_DTC_LEN    6   /* 5 chars + null: "P0300\0" */
#define OPENDASH_VIN_LEN    18  /* 17 chars + null */
...
typedef struct {
    ...
    float    efr_speed;
    float    knock;
    /* ── OBD-II summary (from bytes 58-92 of MD frame, when OBD2 active) ── */
    uint8_t  obd2_flags;        /* bit0=ELM ready, bit1=has DTCs */
    float    obd2_rpm;
    float    obd2_speed;
    ...
    bool     obd2_present;      /* true if OBD2 flags byte ≠ 0 or any PID value ≠ 0 */
    /* ── DTC data (from TAG 0x44 frames) ──────────────────────────────── */
    char     dtc_codes[OPENDASH_MAX_DTCS][OPENDASH_DTC_LEN]; /* e.g. "P0300" */
    uint8_t  dtc_count;         /* number of valid DTCs in dtc_codes[] */
    bool     dtc_data_valid;    /* true after at least one 0x44 frame parsed */
    /* ── MIL status (from TAG 0x4D frames) ────────────────────────────── */
    bool     mil_on;            /* MIL lamp commanded on by ECU */
    uint8_t  mil_dtc_count;     /* DTC count reported by PID 0101 */
    /* ── VIN data (from TAG 0x56 frames) ──────────────────────────────── */
    char     vin[OPENDASH_VIN_LEN];  /* 17-char VIN + null */
    bool     vin_valid;         /* true after valid VIN frame received */
...
/**
 * @brief Send a command to multidisplay via UART TX (Bluetooth back-channel).
 *
 * Requires OPENDASH_UART_TX_PIN >= 0 (TX wire physically connected).
 * Returns false if TX is not configured.
 *
 * @param[in] cmd  Command byte to send (0x44=DTC read, 0x43=DTC clear,
 *                 0x56=VIN read, 0x4D=MIL status)
 * @return true if command was sent, false if TX not available
 */
bool opendash_uart_send_command(uint8_t cmd);

/**
 * @brief Check if UART TX (bidirectional) is available.
 * @return true if TX pin is configured and UART can send
 */
bool opendash_uart_tx_available(void);

/** Suspend the UART RX task (used to quiesce the system during BLE OTA). */
