...
/* Keep BLE-OTA status visible for a grace window even if the node goes
 * OFFLINE immediately after entering OTA (expected: ESP-NOW is deinit'd). */
#define NODE_HEALTH_BLE_OTA_LATCH_MS    360000  /* 6 minutes */
...
typedef struct {
    ...
    /* Last reported STATUS_REPORT flags (OPENDASH_STATUS_FLAG_*).
     * Updated by the dispatcher whenever a STATUS_REPORT arrives.
     * Cleared to 0 when the node transitions to OFFLINE. */
    uint8_t             status_flags;
    uint32_t            ota_latch_until_ms; /**< Preserve BLE_OTA while link is intentionally down */
...
/* ────────────────────────────────────────────────────────────────────────────
 * STATUS_REPORT flag tracking
 *
 * Slaves OR OPENDASH_STATUS_FLAG_* bits into their STATUS_REPORT payload
 * (byte 1, after node_id). The dispatcher stores those bits on the matching
 * health record so the UI can display BLE-OTA / error badges.
 * ──────────────────────────────────────────────────────────────────────────── */

/**
 * @brief Record the most-recent STATUS_REPORT flag byte from a node.
 *
 * Called by the espnow dispatcher whenever a STATUS_REPORT frame arrives.
 * Pure runtime state — not persisted to NVS.
 */
void node_health_set_status_flags(opendash_node_t node, uint8_t flags);

/**
 * @brief Read the most-recent STATUS_REPORT flag byte for a node.
 *
 * @return Flags last seen (or 0 if never reported / invalid node).
 */
uint8_t node_health_get_status_flags(opendash_node_t node);

/**
 * @brief Find the first node currently advertising OPENDASH_STATUS_FLAG_BLE_OTA.
 *
 * Used by the UI to draw a "BT-OTA" banner. If multiple nodes are in OTA
 * mode this returns the lowest-indexed one; the caller can iterate further
 * if it cares about the rest.
 *
 * @param out_node Optional out parameter receiving the node index.
 * @return true if at least one node is in BLE OTA mode, false otherwise.
 */
bool node_health_any_in_ble_ota(opendash_node_t *out_node);
