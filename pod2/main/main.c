...
/**
 * @file main.c
 * @brief OpenDash Pod 2 — Entry Point
 *
 ...
 *
 * Role: ESP-NOW node (OPENDASH_NODE_POD2)
...
/* Splash screen image */
#if __has_include("splash_pod2.h")
#include "splash_pod2.h"
#define HAS_SPLASH 1
#else
#define HAS_SPLASH 0
#endif

static const char *TAG = "opendash_pod2";
        ...
        case OPENDASH_CMD_SYSTEM: {
            ...
            switch (msg->payload[0]) {
                ...
                case OPENDASH_SUBCMD_ENTER_BT_OTA: {
                    ...
                    opendash_bt_ota_enter(OPENDASH_NODE_POD2,
...
static void data_broadcast_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        /* ── Heartbeat STATUS_REPORT every 42s (210 cycles @ 5Hz) — staggered: P1=39 P2=42 ── */
        if ((cycle % 210) == 0) {
            uint8_t hb_payload[3] = { OPENDASH_NODE_POD2, 0x01, 0x00 };
...
static void show_splash_screen(void)
{
    ...
    lv_image_set_src(img, &splash_pod2_dsc);
    lv_obj_center(img);
...
{
    ESP_LOGI(TAG, "═══════════════════════════════════════════════════");
    ESP_LOGI(TAG, "  OpenDash Pod 2 — Display + IMU Unit");
    ...
    ESP_LOGI(TAG, "  Node: OPENDASH_NODE_POD2 (ESP-NOW)");
    ESP_LOGI(TAG, "═══════════════════════════════════════════════════");
    ...
    opendash_err_t od_ret = opendash_config_load(OPENDASH_NODE_POD2, &layout);
    if (od_ret != OPENDASH_OK) {
        ESP_LOGW(TAG, "Config load failed, using defaults");
        opendash_config_reset_defaults(OPENDASH_NODE_POD2, &layout);
    }
    ...
    ESP_ERROR_CHECK(opendash_espnow_init(OPENDASH_NODE_POD2));
    ...
    opendash_rollover_detector_start(OPENDASH_NODE_POD2, rollover_read);

    xTaskCreatePinnedToCore(
        data_broadcast_task, "pod2_bcast", 6144, NULL,
        4,    /* Medium priority */
        NULL,
        0     /* Core 0 */
    );
