...
static SemaphoreHandle_t        s_vsync_sem   = NULL;  /* Vsync sync for tear-free rendering */
...
/* ────────────────────────────────────────────────────────────────────────────
 * GT911 Touch Controller State
 * ──────────────────────────────────────────────────────────────────────────── */
#define GT911_REG_STATUS        0x814E  /* Bit7=ready, Bits3:0=num_touches */
#define GT911_REG_TOUCHES_BASE  0x8150  /* First touch point (8 bytes) */

static uint8_t          s_gt911_addr  = 0;      /* Detected I2C address (0 = not found) */
...
void display_pause_for_ota(void)
{
    ...
    /* RGB DMA on the LCD bounces through PSRAM at high bandwidth; on
     * LEFT/RIGHT this appears to starve the BT controller's HCI link such
     * that LL Connection Complete never reaches NimBLE host. Tearing down
     * the panel here cuts the DMA traffic for the remainder of the OTA
     * window (we esp_restart() at the end either way). */
    if (s_panel) {
        esp_lcd_panel_del(s_panel);
        s_panel = NULL;
        ESP_LOGW(TAG, "RGB panel deleted to free radio bandwidth for BLE OTA");
    }
}
...
/* ═══════════════════════════════════════════════════════════════════════════
 * Section 6 — GT911 Touch Controller
 *
 ...
 *
 * I2C bus sharing: GT911 shares the I2C master bus (port 0, SDA=15, SCL=7)
 * with the TCA9554 IO expander.  We initialize GT911 AFTER all display
 * init is complete so transient SDA driving during GT911 boot cannot
 * corrupt TCA9554 registers while they are being actively written.
 * ═══════════════════════════════════════════════════════════════════════════ */

/**
 * @brief Write to a 16-bit GT911 register via new I2C master API.
 */
static esp_err_t gt911_write_reg(uint16_t reg, const uint8_t *data, size_t len)
{
    if (s_gt911_dev == NULL) return ESP_ERR_INVALID_STATE;
    /* Buffer = [regH, regL, data...]; max 8 byte payload is plenty (we write 1) */
    uint8_t buf[2 + 16];
    if (len > sizeof(buf) - 2) return ESP_ERR_INVALID_SIZE;
    buf[0] = (reg >> 8) & 0xFF;
    buf[1] = reg & 0xFF;
    if (data && len) memcpy(&buf[2], data, len);
    return i2c_master_transmit(s_gt911_dev, buf, 2 + len, I2C_TIMEOUT_MS);
}

/**
 * @brief Read from a 16-bit GT911 register via new I2C master API.
 */
static esp_err_t gt911_read_reg(uint16_t reg, uint8_t *data, size_t len)
{
    if (s_gt911_dev == NULL) return ESP_ERR_INVALID_STATE;
    uint8_t reg_buf[2] = { (reg >> 8) & 0xFF, reg & 0xFF };
    return i2c_master_transmit_receive(s_gt911_dev, reg_buf, sizeof(reg_buf),
                                       data, len, I2C_TIMEOUT_MS);
}

/**
 * @brief Probe a single GT911 address — use i2c_master_probe() on the bus.
 */
static bool gt911_probe_addr(uint8_t addr)
{
    if (s_i2c_bus == NULL) return false;
    esp_err_t ret = i2c_master_probe(s_i2c_bus, addr, I2C_TIMEOUT_MS);
    return (ret == ESP_OK);
}

/**
 * @brief Read one touch point from GT911.
 *
 * GT911 register map:
 *   0x814E — Status (bit7=buffer_ready, bits3:0=num_touches)
 *   0x8150 — Touch point 1 (8 bytes: id, xL, xH, yL, yH, sizeL, sizeH, rsvd)
 */
static esp_err_t gt911_read_touch(uint16_t *x, uint16_t *y, bool *pressed)
{
    *pressed = false;
    *x = 0;
    *y = 0;

    if (s_gt911_addr == 0) return ESP_ERR_INVALID_STATE;
    ...
    uint8_t td[8];
    ret = gt911_read_reg(GT911_REG_TOUCHES_BASE, td, 8);
    if (ret != ESP_OK) return ret;

    uint16_t raw_x = (td[2] << 8) | td[1];
    uint16_t raw_y = (td[4] << 8) | td[3];
...
/**
 * @brief Initialize GT911: release from reset, probe addresses, verify.
 *
 * Called AFTER display init is fully complete (RGB DMA stable).
 *
 * GT911 address selection — determined by INT pin state at RST rising edge:
 *   INT LOW  → address 0x5D
 *   INT HIGH → address 0x14
 *
 * Official Waveshare demo reset sequence (GT911.c → touch_gt911_reset):
 *   1. GPIO16 as output LOW (150ms)
 *   2. EXIO2 (RST) LOW (150ms)
 *   3. EXIO2 (RST) HIGH — rising edge while INT=LOW → selects 0x5D (50ms)
 *   4. GPIO16 HIGH then release to input
 *
 * NOTE: GPIO16 is also used for I2C slave SCL, but display_init() runs
 * BEFORE I2C slave init, so it's safe to use temporarily.
 */
#define GT911_INT_GPIO  GPIO_NUM_16  /* Touch INT — from Waveshare demo */

static esp_err_t touch_probe(void)
{
    ESP_LOGI(TAG, "GT911 touch: Waveshare reset sequence (INT=GPIO%d)...", GT911_INT_GPIO);

    /* Step 1: Drive INT LOW — selects address 0x5D at reset rising edge */
    ...
    gpio_config(&int_cfg);
    gpio_set_level(GT911_INT_GPIO, 0);
    vTaskDelay(pdMS_TO_TICKS(150));

    /* Step 2: RST LOW via TCA9554 EXIO2 */
    tca9554_set_pin(TCA9554_EXIO_TP_RST, false);
    vTaskDelay(pdMS_TO_TICKS(150));

    /* Step 3: RST HIGH — GT911 latches INT=LOW → address 0x5D */
    tca9554_set_pin(TCA9554_EXIO_TP_RST, true);
    vTaskDelay(pdMS_TO_TICKS(50));

    /* Step 4: Release INT — set HIGH then switch to input */
    gpio_set_level(GT911_INT_GPIO, 1);
    int_cfg.mode = GPIO_MODE_INPUT;
    int_cfg.pull_up_en = GPIO_PULLUP_DISABLE;
    gpio_config(&int_cfg);
    vTaskDelay(pdMS_TO_TICKS(50));  /* Extra settling time */

    /* Step 5: Probe addresses */
    if (gt911_probe_addr(TOUCH_I2C_ADDR_A)) {
        s_gt911_addr = TOUCH_I2C_ADDR_A;
        ESP_LOGI(TAG, "GT911 detected at 0x%02X", s_gt911_addr);
    } else if (gt911_probe_addr(TOUCH_I2C_ADDR_B)) {
        s_gt911_addr = TOUCH_I2C_ADDR_B;
        ESP_LOGI(TAG, "GT911 detected at 0x%02X", s_gt911_addr);
    } else {
        /* Full I2C bus scan for diagnostics */
        ESP_LOGW(TAG, "GT911 NOT at 0x5D or 0x14 — scanning I2C bus...");
        for (uint8_t addr = 0x08; addr < 0x78; addr++) {
            if (gt911_probe_addr(addr)) {
                ESP_LOGI(TAG, "  I2C device found at 0x%02X", addr);
            }
        }

        ESP_LOGW(TAG, "GT911 touch disabled — not found on I2C bus");
        tca9554_set_pin(TCA9554_EXIO_TP_RST, false); /* back to reset */
        return ESP_ERR_NOT_FOUND;
    }

    /* Attach GT911 as a new-API device using the detected address */
    i2c_device_config_t gt911_cfg = {
        .dev_addr_length = I2C_ADDR_BIT_LEN_7,
        .device_address  = s_gt911_addr,    /* 0x5D or 0x14, unshifted */
        .scl_speed_hz    = 400000,
    };
    esp_err_t add_ret = i2c_master_bus_add_device(s_i2c_bus, &gt911_cfg, &s_gt911_dev);
    if (add_ret != ESP_OK) {
        ESP_LOGE(TAG, "i2c_master_bus_add_device(GT911) failed: %s", esp_err_to_name(add_ret));
        s_gt911_addr = 0;
        return add_ret;
    }

    /* Refresh TCA9554 state — GT911 boot may have caused transient bus issues */
    vTaskDelay(pdMS_TO_TICKS(10));
    tca9554_write_reg(TCA9554_CONFIG_REG, 0x00);
    tca9554_write_reg(TCA9554_OUTPUT_REG, s_tca_output_shadow);
...
/**
 * @brief Touch reading task — polls GT911 at 50 Hz.
 */
static void touch_read_task(void *arg)
{
    ...
    for (;;) {
        if (gt911_read_touch(&x, &y, &pressed) == ESP_OK) {
            s_touch_x = x;
            s_touch_y = y;
            s_touched = pressed;
        }
...
/**
 * @brief Register LVGL touch input device and start polling task.
 *
 * Only call if touch_probe() found the GT911.
 */
static esp_err_t start_touch_input(void)
{
    /* Register LVGL indev — must be done while LVGL lock is NOT held,
     * since lv_indev_create() is safe to call outside the lock. */
    s_touch_indev = lv_indev_create();
    if (!s_touch_indev) {
        ESP_LOGE(TAG, "Failed to create LVGL touch indev");
        return ESP_FAIL;
    }
    lv_indev_set_type(s_touch_indev, LV_INDEV_TYPE_POINTER);
    lv_indev_set_read_cb(s_touch_indev, touch_input_cb);
    lv_indev_set_display(s_touch_indev, s_lvgl_disp);

    /* Start polling task on Core 0 */
    BaseType_t ret = xTaskCreatePinnedToCore(
        touch_read_task, "touch_rd", 3072, NULL,
        3,      /* above UI task priority */
        NULL, 0 /* Core 0 */
    );
    if (ret != pdPASS) {
        ESP_LOGE(TAG, "Failed to create touch read task");
        return ESP_FAIL;
    }

    ESP_LOGI(TAG, "GT911 touch input registered with LVGL");
    return ESP_OK;
}
...
esp_err_t display_init(void)
{
    ...
    /* Step 10: TCA9554 recovery — MUST run BEFORE touch probe.
     * RGB panel DMA can corrupt TCA9554 CONFIG register to 0xFF (all inputs),
     * making EXIO2 (GT911 RST) float instead of drive high.
     * We must ensure TCA9554 outputs are stable before touch_probe() uses EXIO2. */
    ...
    /* Step 10b: GT911 touch probe — NOW safe because TCA9554 is stable.
     * EXIO2 (GT911 RST) can be reliably driven by tca9554_set_pin().
     * Retry up to 3 times if probe fails — DMA bus transients may
     * still interfere with the first attempt. */
    for (int retry = 0; retry < 3; retry++) {
        if (touch_probe() == ESP_OK) break;
        ESP_LOGW(TAG, "GT911 probe attempt %d failed, retrying after TCA9554 refresh...", retry + 1);
        tca9554_write_reg(TCA9554_CONFIG_REG, 0x00);
        tca9554_write_reg(TCA9554_OUTPUT_REG, s_tca_output_shadow);
    ...
    /* Step 12: GT911 touch input — register LVGL indev + start polling task.
     * Only if touch_probe() found the GT911 (s_gt911_addr != 0). */
    if (s_gt911_addr != 0) {
        start_touch_input();
    }
