...
# OpenDash — Left Gauge Pod (2.8" Round LCD)

ESP-IDF project for the Left gauge pod in the OpenDash racecar dashboard system.
...
The `/left` and `/right` directories contain identical firmware. The only differences:

| Item | Left | Right |
|---|---|---|
| I2C Address | `0x10` | `0x11` |
| Node Type | `OPENDASH_NODE_LEFT` | `OPENDASH_NODE_RIGHT` |
| Default Primary | Oil Pressure | Boost |
| Default Secondary | Oil Temp | AFR |
| Background Image | `background_left` | `background_right` |
...
**OpenDash Left Gauge** — Built for racers, by racers.
