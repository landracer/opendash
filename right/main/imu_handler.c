...
/**
 * @file imu_handler.c
 * @brief OpenDash IMU Handler (RIGHT) — QMI8658 via I2C
 *
 * Register-level QMI8658 driver, identical to pod1/pod2/gps, ported to RIGHT
 * so it can participate as a rollover-detection voter. If the chip is absent
 * (a RIGHT board built without the IMU populated), init fails SOFT: the display
 * keeps running and imu_handler_is_present() returns false so RIGHT never
 * advertises a bogus vote.
 *
...
{
    return i2c_master_transmit_receive(qmi8658_handle, &reg, 1, value, 1, 100);
}
...
{
    if (!s_present) {
        ESP_LOGW(TAG, "IMU not present — task not started (RIGHT will not vote)");
