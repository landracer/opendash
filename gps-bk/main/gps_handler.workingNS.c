...
static void gps_ensure_power(i2c_master_bus_handle_t bus)
{
    ...
    /* Assert NRESET LOW via TCA9554 P5 before cutting power */
    ...
    /* PMIC: Turn OFF GPS rails, wait, turn ON */
    ...
    if (i2c_master_bus_add_device(bus, &pmu_cfg, &pmu) == ESP_OK) {
        ...
        ESP_LOGI(TAG, "  GPS rails OFF — waiting 2s");
    ...
    /* Wait for LC76G boot */
    ESP_LOGI(TAG, "  Waiting 3s for LC76G boot...");
...
static void gps_task(void *pvParameters)
{
    ...
    /* ── Warm-up: initial bus reset + verify 0x50 responds ── */
    ...
    /* ── Send initial WAKE to activate I2C slave interface ──
     * Per official Waveshare/Quectel demo (Ql_Wake_I2C):
     * Write a dummy 0x00 byte through the CASIC write protocol.
     * This wakes the I2C TX buffer so it starts filling with NMEA data. */
    for (int w = 0; w < 3; w++) {
        i2c_master_bus_reset(bus);
    ...
    /* ── Check initial avail after wake ── */
    for (int i = 0; i < 5; i++) {
        i2c_master_bus_reset(bus);
        vTaskDelay(pdMS_TO_TICKS(50));
    ...
    while (1) {
        ...
        if (rx_ret != ESP_OK) {
            ...
            /* If RX from 0x54 fails, try activation via transmit_receive on 0x50 */
            if (consecutive_fails > 0 && (consecutive_fails % 5 == 0)) {
                ...
                ESP_LOGW(TAG, "  Activation TxRx(0x50): %s [%02X%02X%02X%02X]",
        ...
        if (dataLength == 0) {
            ...
            /* ── WAKE mechanism (from official Waveshare demo) ──
             * After 30 consecutive empty polls, write a dummy byte to wake
             * the LC76G I2C slave interface. */
            if (empty_polls >= 30) {
                uint8_t wake_cw[] = {0x00, 0x10, 0x53, 0xAA, 0x01, 0x00, 0x00, 0x00};
                ...
                ESP_LOGI(TAG, "  I2C WAKE sent (empty_polls=%lu)", (unsigned long)empty_polls);
                empty_polls = 0;
                vTaskDelay(pdMS_TO_TICKS(100));
            }

            if (cycle < 20 || (cycle % 100) == 0)
                ESP_LOGI(TAG, "avail=0 (c=%lu, empty=%lu)", (unsigned long)cycle, (unsigned long)empty_polls);
            cycle++;
            vTaskDelay(pdMS_TO_TICKS(100));
            continue;
        }
        ...
        empty_polls = 0;  /* Reset wake counter on valid data */
        ...
        if (tx_ret != ESP_OK) {
            ESP_LOGW(TAG, "data req TX failed: %s (c=%lu)",
...
esp_err_t gps_handler_init(void)
{
    ...
    /* ── GPS PMIC power cycle — required after software reset ── */
