...
static node_info_t s_nodes[MAX_NODES] = {
    ...
    { .node = OPENDASH_NODE_BMS,        .online = false, .mac_known = false },
    { .node = OPENDASH_NODE_MOS_4CH_A,  .online = false, .mac_known = false },
    { .node = OPENDASH_NODE_MOS_4CH_B,  .online = false, .mac_known = false },
};
...
/** @brief Set true when real engine data arrives from Left pod (via MD).
 *  Suppresses the drag-race demo generator for engine data points. */
static bool s_real_engine_data_received = false;

/* Scratch buffer for protocol serialization (single-threaded use) */
static uint8_t s_tx_buf[OPENDASH_ESPNOW_MAX_DATA];

/* ----------------------------------------------------------------------------
 * Auxiliary RX callback (used by boost_client to consume boost frames).
 * --------------------------------------------------------------------------*/
...
/* ----------------------------------------------------------------------------
 * DP cache (latest values seen for every relevant DP).
 *
 * Reads from this cache feed the boost controller's live engine snapshot at
 * 10 Hz. Writers: process_responses() (real telemetry) + the demo path.
 * Single-writer / multi-reader — ESP32 32-bit aligned word writes are atomic
 * enough for our needs, so no mutex.
 * --------------------------------------------------------------------------*/
typedef struct {
    volatile float    rpm;
    volatile float    boost_kpa;     /**< Manifold pressure, gauge kPa */
    volatile float    egt_c;         /**< Hottest EGT */
    volatile float    afr;
    volatile float    fuel_kpa;
    volatile float    throttle_pct;
    volatile float    gear;
    volatile uint32_t last_update_ms;
} dp_cache_t;

static dp_cache_t s_dp_cache = {0};

/** @brief Mirror a DP update into s_dp_cache. Called from the same task
 *  that runs flush_pending_ui_updates() so no locking needed. */
static inline void dp_cache_update(uint16_t dp_id, float value)
{
    switch (dp_id) {
        case OPENDASH_DP_RPM:            s_dp_cache.rpm          = value; break;
        case OPENDASH_DP_BOOST_PRESSURE: s_dp_cache.boost_kpa    = value; break;
        case OPENDASH_DP_EGT1:           /* fall through — we want hottest */
        case OPENDASH_DP_EGT2:
        case OPENDASH_DP_EGT3:
        case OPENDASH_DP_EGT4:
            if (value > s_dp_cache.egt_c) s_dp_cache.egt_c = value;
            break;
        case OPENDASH_DP_AFR:            s_dp_cache.afr          = value; break;
        case OPENDASH_DP_FUEL_PRESSURE:  s_dp_cache.fuel_kpa     = value; break;
        case OPENDASH_DP_THROTTLE_POS:   s_dp_cache.throttle_pct = value; break;
        case OPENDASH_DP_GEAR:           s_dp_cache.gear         = value; break;
        default: return;
    }
    s_dp_cache.last_update_ms = (uint32_t)(esp_timer_get_time() / 1000);
}

/** @brief Public-ish accessor (declared in boost_client.c as extern). */
void espnow_master_snapshot_engine(float *rpm, float *boost_kpa, float *egt_c,
                                    float *afr, float *fuel_kpa,
                                    float *throttle_pct, float *gear,
                                    uint32_t *last_update_ms)
{
    if (rpm)            *rpm            = s_dp_cache.rpm;
    if (boost_kpa)      *boost_kpa      = s_dp_cache.boost_kpa;
    if (egt_c)          *egt_c          = s_dp_cache.egt_c;
    if (afr)            *afr            = s_dp_cache.afr;
    if (fuel_kpa)       *fuel_kpa       = s_dp_cache.fuel_kpa;
    if (throttle_pct)   *throttle_pct   = s_dp_cache.throttle_pct;
    if (gear)           *gear           = s_dp_cache.gear;
    if (last_update_ms) *last_update_ms = s_dp_cache.last_update_ms;
}

bool espnow_master_node_online(opendash_node_t node)
{
    for (int i = 0; i < MAX_NODES; ++i) {
        if (s_nodes[i].node == node) return s_nodes[i].online;
    }
    return false;
}
...
{
    /* Build a single i2c-format frame and unicast (or broadcast if MAC
     * not yet known). Used by boost_client to push maps + params. */
    opendash_i2c_msg_t m;
    opendash_i2c_build_msg(&m, cmd, (uint8_t *)payload, len);
    uint8_t buf[OPENDASH_ESPNOW_MAX_DATA]; uint16_t blen = 0;
    if (opendash_i2c_serialize(&m, buf, &blen) != OPENDASH_OK) return ESP_FAIL;
    for (int i = 0; i < MAX_NODES; ++i) {
        if (s_nodes[i].node == node) {
            if (s_nodes[i].mac_known) return opendash_espnow_send(s_nodes[i].mac, buf, blen);
            return opendash_espnow_broadcast(buf, blen);
        }
    }
                                  ...
                                  const opendash_i2c_msg_t *msg)
{
    ...
    /* Detect real engine data — suppress demo generator once live data flows */
    if (!s_real_engine_data_received &&
        (dp_id == OPENDASH_DP_RPM || dp_id == OPENDASH_DP_BOOST_PRESSURE ||
         dp_id == OPENDASH_DP_COOLANT_TEMP)) {
        s_real_engine_data_received = true;
        ESP_LOGI(TAG, "Real engine data detected (dp=0x%04X) — demo halted", dp_id);
    }
...
static void flush_pending_ui_updates(void)
{
    ...
    for (int i = 0; i < s_pending_ui_count; i++) {
        ...
        /* Mirror to boost-controller DP cache before/regardless of LVGL */
        dp_cache_update(s_pending_ui[i].dp_id, s_pending_ui[i].value);
...
static void espnow_master_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        s_node_status.bms_online   = s_nodes[3].online;

        /* ── Phase 4: Drag race demo data + push to all displays ─── */
#if OPENDASH_DEMO_DATA
        if (!s_real_engine_data_received)
        {
            float rpm, speed_mph, boost, coolant, oil, batt, afr;
            drag_race_values(&rpm, &speed_mph, &boost, &coolant, &oil, &batt, &afr);

            /* Log engine demo values */
            sd_logger_log_datapoint(OPENDASH_DP_RPM, rpm);
            sd_logger_log_datapoint(OPENDASH_DP_COOLANT_TEMP, coolant);
            sd_logger_log_datapoint(OPENDASH_DP_OIL_TEMP, oil);
            sd_logger_log_datapoint(OPENDASH_DP_BATTERY_VOLTAGE, batt);
            sd_logger_log_datapoint(OPENDASH_DP_BOOST_PRESSURE, boost);
            sd_logger_log_datapoint(OPENDASH_DP_AFR, afr);

            /* Batch all LVGL updates in a single lock (minimizes
             * display latency — all values update in the same frame). */
            if (display_lvgl_lock(10)) {
                /* Engine demo — always active */
                ...
                /* Mirror to boost DP cache so the controller has something to
                 * chew on even without a real ECU. */
                dp_cache_update(OPENDASH_DP_RPM, rpm);
                dp_cache_update(OPENDASH_DP_BOOST_PRESSURE, boost);
                dp_cache_update(OPENDASH_DP_AFR, afr);

                /* GPS demo data — ONLY when GPS node is offline.
                 * When online, real GPS data flows via Phase 2
                 * (process_responses → flush_pending_ui_updates). */
                if (!s_nodes[2].online) {
                    float demo_t     = (float)(esp_timer_get_time()) / 1000000.0f;
                    float gps_speed  = speed_mph;
                    float altitude   = 150.0f + speed_mph * 0.3f;
                    float sat_count  = 12.0f;
                    float heading    = fmodf(demo_t * 5.0f, 360.0f);
                    float hdop       = 0.8f;
                    ...
                    ui_manager_update_value(OPENDASH_DP_GPS_FIX, 0.0f);
                }
                display_lvgl_unlock();
            }
            esp_task_wdt_reset();
            ...
            for (int i = 0; i < 2; i++) {
                ...
                push_data_point(&s_nodes[i], OPENDASH_DP_GPS_SPEED, speed_mph);
            }
        }
#endif /* OPENDASH_DEMO_DATA */
...
esp_err_t espnow_master_init(void)
{
    ...
    /* Initialize SD card logger (mounts TF card, loads session counter) */
    esp_err_t log_ret = sd_logger_init();
    if (log_ret != ESP_OK) {
        ESP_LOGW(TAG, "SD logger init failed: %s (logging disabled)",
