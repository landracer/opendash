...
static int64_t                     s_telem_last_us = 0;
    ...
    switch (msg->cmd) {
        case OPENDASH_CMD_BOOST_TELEMETRY:
            if (msg->length >= sizeof(opendash_boost_telemetry_t)) {
                ...
                s_telem_valid    = true;
                s_telem_last_us  = esp_timer_get_time();
...
{
    ESP_LOGI(TAG, "boost_live_push_task started @ 20 Hz");
    /* 20 Hz: with the slave's 600 ms DATA_STALE window this gives ~12 frames of
     * slack, so a transient ESP-NOW gap no longer trips a visible LIMP cut. */
    ...
    while (1) {
        ...
        if (g_boost_target_node >= OPENDASH_NODE_COUNT) continue;
...
uint32_t boost_client_telemetry_age_ms(void)
{
    ...
    xSemaphoreTake(s_lock, portMAX_DELAY);
    if (s_telem_valid) {
        int64_t dt = esp_timer_get_time() - s_telem_last_us;
        if (dt < 0) dt = 0;
        if (dt > (int64_t)UINT32_MAX * 1000LL) age = UINT32_MAX;
        else age = (uint32_t)(dt / 1000);
    }
    xSemaphoreGive(s_lock);
    return age;
}
...
{
    if (!s_lock) return 0;
    if (node != g_boost_target_node) return 0;
    uint8_t m = 0;
    xSemaphoreTake(s_lock, portMAX_DELAY);
    if (s_params_valid) m = s_params.output_mask & 0x0Fu;
    xSemaphoreGive(s_lock);
    return m;
}
