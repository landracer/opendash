 ...
 *   - 0x58-ONLY WAKE (v16c): CW config writes to 0x50 were removed — they
 *     corrupt the LC76G CASIC command parser after ~88 cumulative writes,
 *     causing the module to NACK everything (the "sees sats then freezes"
 *     lockup). A dummy write to 0x58 supplies the keep-alive bus activity
 *     without touching the command parser. See wiki/gps-driver-debugging-v16.md.
 *   - PMIC read guard (v16j): validate AXP2101 reg 0x90 before clearing rails
...
static void gps_ensure_power(i2c_master_bus_handle_t bus)
{
    ...
    if (i2c_master_bus_add_device(bus, &pmu_cfg, &pmu) == ESP_OK) {
        uint8_t ldo = 0;
        esp_err_t ldo_ret = axp2101_read_reg(pmu, 0x90, &ldo);
        ESP_LOGI(TAG, "  PMIC 0x90 before: 0x%02X", ldo);

        /* v16j: Guard against a garbage read. On the contended shared bus the
         * read can return 0x00; then ldo & ~0x2C == 0x00, which writes ALL LDO
         * rails OFF — including the ESP32's own supply — killing the board
         * (blank AMOLED, USB drop, needs physical power removal). Fall back to
         * a safe all-on default so we only ever clear the GPS rails.
         * See wiki/gps-driver-debugging-v16.md §2.3 (Root Cause #3). */
        if (ldo_ret != ESP_OK || ldo == 0x00) {
            ESP_LOGW(TAG, "  PMIC 0x90 read invalid (0x%02X, %s) — safe default 0xFF",
                     ldo, esp_err_to_name(ldo_ret));
            ldo = 0xFF;
        }
...
static void gps_full_recovery(i2c_master_bus_handle_t bus)
{
    ...
    /* 3 WAKE cycles — v16c: 0x58-ONLY (CW to 0x50 removed; see §2.1) */
    for (int w = 0; w < 3; w++) {
...
static void gps_task(void *pvParameters)
{
     ...
     * corrupts the LC76G CASIC command parser after ~88 cumulative writes,
     * after which the module NACKs everything and "freezes". A dummy write to
     * 0x58 provides the bus activity that keeps the I2C slave alive without
     * touching the command parser. See wiki/gps-driver-debugging-v16.md §2.1. */
    for (int w = 0; w < 3; w++) {
        i2c_master_bus_reset(bus);
        vTaskDelay(pdMS_TO_TICKS(50));
        ...
        ESP_LOGI(TAG, "  Wake[%d]: dw=%s", w, esp_err_to_name(dw));
        vTaskDelay(pdMS_TO_TICKS(500));
    }
    ...
    while (1) {
        ...
        if (dataLength == 0) {
            ...
            /* See wiki §5.2 — WAKE after 5 empty polls */
            /* v16c: 0x58-ONLY WAKE (CW to 0x50 removed — it corrupts the CASIC
             * parser after ~88 writes; see §2.1). */
            if (empty_polls >= 5) {
                uint8_t dummy = 0x00;
        ...
        /* v16c: 0x58-ONLY WAKE after every successful read. This was the single
         * worst offender — firing the CW config write on every read crossed the
         * ~88-write corruption threshold within seconds, which is exactly the
         * "sees sats then freezes" failure. The 0x58 dummy write alone keeps the
         * I2C slave alive without touching the command parser. See §2.1. */
        {
            uint8_t wd = 0x00;
            i2c_master_transmit(lc76g_dwr_handle, &wd, 1, 500);
            current_gps_debug.total_wakes++;
