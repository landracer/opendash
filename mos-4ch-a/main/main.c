                              ...
                              const opendash_i2c_msg_t *msg)
{
    ...
    switch (msg->cmd) {
        ...
        case OPENDASH_CMD_SYSTEM: {
            ...
            switch (msg->payload[0]) {
                ...
                case OPENDASH_SUBCMD_ENTER_BT_OTA:
                    ...
                    /* Safe shutdown: kill PWM on every boost-capable channel, disable all FETs */
                    for (int _c = 0; _c < 4; _c++) opendash_relay_set_pwm(_c, 0);
                    opendash_relay_all_off();
                    opendash_espnow_deinit();
                    opendash_bt_ota_start(OPENDASH_NODE_MOS_4CH_A);
                    vTaskDelay(pdMS_TO_TICKS(100));
                    esp_restart();
                    break;
                default:
        ...
        case OPENDASH_CMD_BOOST_PULL_ALL: {
            /* Center requests a full dump: params + every duty/setp row +
             * throttle curve.  Stagger sends with small delays so we don't
